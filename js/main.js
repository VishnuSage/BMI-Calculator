...
function weightClassRange() {
  let bmi2 = (
    calculateWeightConversion(st.value, lbs.value) /
    calculateHeightConversion(ft.value, inch.value)
  ).toFixed(1);
  ...
  let lowerWeight =
    18.5 * calculateHeightConversion(ft.value, inch.value) * 2.205;
  let lowerWeightSt = Math.floor(lowerWeight * 0.0714) + "st";
  let lowerWeightPound = Math.floor(lowerWeight % 14) + "lbs";

  let upperWeight =
    24.9 * calculateHeightConversion(ft.value, inch.value) * 2.205;
...
const inputMetricForm = document.getElementById("inputMetric");
const inputImperialForm = document.getElementById("inputImperial");

metricRadio.addEventListener("change", () => {
  inputMetricForm.reset();
  inputImperialForm.reset();
});
